...
def diagonalProduct(matrix):
    ...
    for i in range(0, 20):
        for j in range(0, 20):
            list = [matrix[i][j], matrix[(i + 1) % 20][(j + 1) % 20],
                    matrix[(i + 2) % 20][(j + 2) % 20], matrix[(i + 3) % 20][(j + 3) % 20]]
            productTotal = 1
            for product in list:
                productTotal *= product
                if greatestProduct < productTotal:
                    greatestProduct = productTotal
                    print("Diag", list)
...
def upDownProduct(matrix):
    greatestProduct = 1
    for i in range(0, 20):
        for j in range(0, 20):
            list = [matrix[i][j], matrix[(i + 1) % 20][j],
                    matrix[(i + 2) % 20][j], matrix[(i + 3) % 20][j]]
            productTotal = 1
            for product in list:
                productTotal *= product
                if greatestProduct < productTotal:
...
def leftRightProduct(matrix):
    greatestProduct = 1
    for i in range(0, 20):
        for j in range(0, 20):
            list = [matrix[i][j], matrix[i][(j + 1) % 20],
                    matrix[i][(j + 2) % 20], matrix[i][(j + 3) % 20]]
            productTotal = 1
            for product in list:
                productTotal *= product
                if greatestProduct < productTotal:
...
products = []
products.append(diagonalProduct(matrixVals))
products.append(revDiagonalProduct(matrixVals))
products.append(upDownProduct(matrixVals))
products.append(leftRightProduct(matrixVals))
